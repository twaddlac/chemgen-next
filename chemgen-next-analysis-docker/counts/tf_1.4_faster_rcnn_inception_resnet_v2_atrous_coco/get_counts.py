...
if not os.path.isfile(FLAGS.image_path) and os.path.isdir(FLAGS.image_path):
...
size = False
results = []
if os.path.exists(FLAGS.counts):
    size = os.stat(FLAGS.counts).st_size != 0

if size is not False and os.path.exists(FLAGS.counts):
    try:
        df = pd.read_csv(FLAGS.counts)
