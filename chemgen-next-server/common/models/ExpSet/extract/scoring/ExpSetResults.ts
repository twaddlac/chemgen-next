...
import {memoize, get, groupBy, shuffle, isEqual, find, round, filter, isObject} from 'lodash';
...
ExpSet.extract.genImageMeta = function (data: ExpSetSearchResults, expAssay2reagents: ExpAssay2reagentResultSet[]) {
  ...
  return expAssay2reagents.map((expAssay2reagent: ExpAssay2reagentResultSet) => {
    ...
    const expSet = ExpSet.extract.getExpSet(data, expAssay);
    if (!treatmentGroupId) {
      const expDesigns = getTreatmentGroupIdFromDesign(expAssay2reagent.expGroupId);
      if (expDesigns) {
